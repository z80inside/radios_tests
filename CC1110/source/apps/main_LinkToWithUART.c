...
void main (void)
{
  ...
  tx_send_wait("\r\nPress [Enter] in HyperTerminal to continue...", sizeof("\r\nPress [Enter] in HyperTerminal to continue...")); 
  ...
  while(true)
  {
    if(rx_peek())                 /* Check for any unread data in queue */
    {
      if(rx_receive(&rxByte, 1)) /* Pop one byte and put into 'rxByte' */
      {
        ...
        if(rxByte == 0x0D)       /* 0x0D is [ENTER] key in ASCII */
        {
          break;
        }
        else
          toggleLED(1);          /* If wrong key, toggle LED */
      }
    }
  }
