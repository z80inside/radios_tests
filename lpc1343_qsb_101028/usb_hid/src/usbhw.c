...
// CodeRed - prevent inlining of 'delay' with GNU compiler.
#if defined(__GNUC__)
void delay (uint32_t length ) __attribute__((noinline));
#endif
...
uint32_t USB_ReadEP (uint32_t EPNum, uint8_t *pData) {
  ...
  for (n = 0; n < (cnt + 3) / 4; n++) {
// CodeRed - cope with GNU way of packing vs RVCT packing
#if defined(__GNUC__)
